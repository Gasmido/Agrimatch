...
<script>
    ...
    if ($subscriptionResult) { 
        // Fetch user's verification status
        $sql = "SELECT topics FROM users WHERE id = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("i", $user_id);
        $stmt->execute();
        $result3 = $stmt->get_result();
        $userss = $result3->fetch_assoc();
?>
    <p><strong>Favorite Topic:</strong> <?php echo htmlspecialchars($userss['topics']); 
    if ($userss['topics'] == NULL) {echo " N/A";}
?></p>
<?php
    }
?>
        ...
        <a href='message.php?question_owner_id=<?= $_SESSION['user_id'] ?>&user_id=<?= $_GET['user_id'] ?>'>Send Message</a>
...
<div style="margin-top:30px;margin-bottom: 30px;">
    <h1><?php echo htmlspecialchars($user['username']); ?>'s Posts</h1>
    <hr>
